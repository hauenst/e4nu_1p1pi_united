...
void Subtraction::prot1_pi3_rot_func(TVector3 V3prot, TVector3 V3pi[3], int q_pi[3], double P_tot[3]){
    const int N_pi=3;
      ...
      for(int i=0; i<3; i++)
        {
          for(int j=0; j<3; j++)
          {
            if(i<j)
            {
              ...
              q_pi2[0] = q_pi[i];
              q_pi2[1] = q_pi[j];
              prot1_pi2_rot_func(V3prot, V3pi2, q_pi2, P_1p1pi);
...
void Subtraction::prot2_pi1_rot_func(TVector3 V3_2prot_corr[2],TVector3 V3_2prot_uncorr[2],TVector3 V3_1pi, int q_pi, double P_tot[2]){
...
void Subtraction::prot2_pi2_rot_func(TVector3 V3_2prot_corr[2],TVector3 V3_2prot_uncorr[2],TVector3 V3_2pi[2], int q_pi[2], double P_tot_2p[2][2]){
    ...
    for(int g=0; g<N_tot; g++){
     ...
     //---------------------------------------------------2p2pi->1p2pi-------------------------------------------------------
      ...
      for(int i=0;i<2;i++)
      {
        prot1_pi2_rot_func(V3_2prot_uncorr[i], V3_2pi, q_pi, P_tot);
    ...
    for(int i=0;i<2;i++)
    {
      prot2_pi1_rot_func(V3_2prot_corr, V3_2prot_uncorr, V3_2pi[i], q_pi[i], P_tot);
...
void Subtraction::prot3_pi1_rot_func(TVector3 V3_3prot_corr[3],TVector3 V3_3prot_uncorr[3],TVector3 V3_pi, int q_pi,double P_tot_3p[3]){
   ...
   for(int i=0;i<N_3prot;i++){       //looping through 2p combinations  out of 3p
     ...
     {               // 3 pairs of 2proton combinations with z, i indexes(z<i)
          ...
          prot2_pi1_rot_func(V3_prot_corr,V3_prot_uncorr,V3_pi, q_pi, P_2p1pito1p1pi);
